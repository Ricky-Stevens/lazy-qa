...
- Read storage. `mcp__browser__evaluate` with `JSON.stringify({local: Object.entries(localStorage), session: Object.entries(sessionStorage)})`. Look for tokens, PII, secrets in plaintext.
