/**
 * run-v2: orchestrator entry point for the v2 architecture.
 *
 * Replaces v1's `run.ts`. Mirrors the same lifecycle (config → safety →
 * agents → reviewer) but inserts the deterministic crawler step BEFORE any
 * agent spawns. The crawler builds a shared `SiteMap` that every agent
 * consumes via `SiteMapAccessor`. After all agents finish, run-v2 writes a
 * `coverage.md` aggregating sitemap stats + (eventually) per-agent playbook
 * outcome traces.
 *
 * v1's `run.ts` stays untouched — `bin/regress.ts` will route to v1 under
...
import { spawnAgentV2 } from './spawn-agent-v2.ts';
...
/** v2 crawler defaults. Mirrors spec §7.1 — the user can override via plugin
...
export async function runScanV2(opts: RunV2Options): Promise<RunV2Result> {
  ...
  // 2. Resolve API key + agents. v2's loop requires a real API key (see
  // spawn-agent-v2 docstring); subscription auth is unsupported.
  const apiKey = resolveApiKey(cfg);
  if (!apiKey) {
    throw new Error(
      'runScanV2 requires ANTHROPIC_API_KEY. The v2 direct-API loop does not support subscription auth via the `claude` CLI. Set ANTHROPIC_API_KEY or use --legacy to fall back to v1.',
    );
  }
  ...
  if (!firstAgent) {
    throw new Error('runScanV2: no agents resolved from config');
  }
  ...
  // wiring per-agent outcome jsonl is owned by WP15. Once that lands, run-v2
  ...
  // 15. Run-level budget warning (informational only — can't enforce mid-run in v2 either).
